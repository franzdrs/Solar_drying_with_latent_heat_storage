module file_data_module
    ...
    double precision, allocatable :: Hour_dec(:), t_min(:), Ins(:), Tamb(:), wind(:), Patm(:), Xa_in(:), Ta_in(:), Avg_Tpcm(:), Tot_Enthlp(:), Lambda_avg(:), mdot_a(:)
...
subroutine read_file_LHS_output(input_file)
    ...
    allocate(Xa_in(0:num_rows-1))
    allocate(Ta_in(0:num_rows-1))
    allocate(Avg_Tpcm(0:num_rows-1))
    allocate(Tot_Enthlp(0:num_rows-1))
    allocate(Lambda_avg(0:num_rows-1))
    allocate(mdot_a(0:num_rows-1))
    ...
    do i = 0, num_rows-1
        ...
        !print *, line
        ! Parse the line and store values in arrays
        read(line, *) Date(i), Hour_min(i), Hour_dec(i), t_min(i), Ins(i), Tamb(i), wind(i), Patm(i), Xa_in(i), Ta_in(i), Avg_Tpcm(i), Tot_Enthlp(i), Lambda_avg(i), mdot_a(i)
    end do
...
double precision function saturation_vapor_pressure(Tad)
    implicit none
    double precision :: Tad, Tabs, log_Pvsat
    Tabs = Tad + 273.15
...
double precision function equilibrium_rh(C1,C2,C3,Ta,Xp)
    implicit none
    double precision :: C1, C2, C3, Ta, Xp
    equilibrium_rh = (1./(((C1 + C2*Ta)/Xp)**C3 + 1.))
end function equilibrium_rh

! FUNCTION TO CALCULATE THE EQUILIBRIUM MOISTURE CONTENT BASED ON SORPTION ISOTHERM EQUATION
double precision function equilibrium_Xp(C1,C2,C3,Ta,rh)
    implicit none
    double precision :: C1, C2, C3, Ta, rh
    equilibrium_Xp = (C1 + C2*Ta)*(rh/(1.-rh))**(1./C3)
...
program Deep_Bed_Drying
    ...
    double precision :: pvsat_Ta, pv_Ta, pv_Ta_in, pvsat_Ta_in, rh, rh0, pvsat0, pv0, Xa0, A, B, C
    double precision :: saturation_vapor_pressure, equilibrium_rh, equilibrium_Xp, drying_rate_K_parameter, drying_rate_N_parameter 
    ...
    do while (Curr_sim_time < Total_sim_t)
        ...
        do while (maxval(delta_Ta) > TOL_delta_Ta .or. maxval(delta_Xa) > TOL_delta_Xa .or. maxval(delta_Tp) > TOL_delta_Tp .or. maxval(delta_Xp) > TOL_delta_Xp)
            ...
            do j = 0,M
                ...
                K = drying_rate_K_parameter(Ta_new(j),rh,v_a)   ! Drying rate K parameter
                N = drying_rate_N_parameter()                ! Drying rate N parameter
