module file_data_module
    ...
    double precision, allocatable :: Hour_dec(:), time_row(:), Ins(:), Tamb(:), rh_amb(:), wind(:), Patm(:), Tf_in(:), Xf_in(:), Tdewp(:), mdot_a(:)
...
subroutine read_file(input_file, start_row, end_row)
    ...
    allocate(Tf_in(0:number_rows_to_read - 1))
    allocate(Xf_in(0:number_rows_to_read - 1))
    ...
    do i = 0, number_rows_to_read - 1
        ...
        read(line, *) Date(i), Hour_min(i), Hour_dec(i), time_row(i), Ins(i), Tamb(i), rh_amb(i), wind(i), Patm(i), Tf_in(i), Xf_in(i), mdot_a(i)
    end do
...
program SAH
    ...
    double precision :: mdot_f, Vdot, Tf_inlet, Xf_inlet
    ...
    double precision :: UsefulEnergyTotal, Total_I, Usef_Q_rate, Efficiency
    double precision :: Count_stor_dt
    ...
    UsefulEnergyTotal = 0.            ! variable to keep track of useful energy over ambient T
    ...
    write(output_unit_number, '(A,A12,F9.3,F8.1,F8.1,F8.1,F8.1,F8.0,F8.4,F13.3,F12.8)') Date(0), Hour_min(0), Hour_dec(0), Curr_sim_time/60., &
        Ins(0), Tamb(0), wind(0), Patm(0), Xf_in(0), Tf(Nx), mdot_a(0)
    write(output_unit_number, '(A,A12,A9,A8,A8,A8,A8,A8,A8,A13,A12)') "Date", "Hour_min", "Hour_dec", "t_min", "Ins", "Tamb", "Wind", "Patm", "X", &
        "Tf_out", "mdot_a"
    ...
    write(output_DETAILS_unit_number, '(A,A12,A9,A8,A8,A8,A9,A9,A8,A8,A8,A9,A15,A12,A11)', advance = 'no') "Date", "Hour_min", "Hour_dec", "t_min", "Ins", "Tamb", "rh_amb", "Wind", "Patm", &
        "Tf_in", "Xf_in", "Tf_out", "Usef_Q_rate", "Coll_Eff", "mdot_a"
    
    do j = 0,size(x_position)-1
        write(real_string, '(F8.2)') x_position(j)
        temp_string = " G" // trim(adjustl(real_string))
        write(output_DETAILS_unit_number, '(A)', advance='no') trim(temp_string)
        !write(output_DETAILS_unit_number, '(A,F8.2)', advance = 'no') " G", x_position(j)
    end do
    do j = 0,size(x_position)-1
        write(real_string, '(F8.2)') x_position(j)
        temp_string = " P" // trim(adjustl(real_string))
        write(output_DETAILS_unit_number, '(A)', advance='no') trim(temp_string)
        !write(output_DETAILS_unit_number, '(A,F8.2)', advance = 'no') "P", x_position(j)
    end do  
    do j = 0,size(x_position)-1
        write(real_string, '(F8.2)') x_position(j)
        temp_string = " F" // trim(adjustl(real_string))
        write(output_DETAILS_unit_number, '(A)', advance='no') trim(temp_string)
        !write(output_DETAILS_unit_number, '(A,F8.2)', advance = 'no') "F", x_position(j)
    end do
    do j = 0,size(x_position)-1
        write(real_string, '(F8.2)') x_position(j)
        temp_string = " B" // trim(adjustl(real_string))
        write(output_DETAILS_unit_number, '(A)', advance='no') trim(temp_string)
        !write(output_DETAILS_unit_number, '(A,F8.2)', advance = 'no') "B", x_position(j)
    end do
    
    write(output_DETAILS_unit_number, *)
    
    write(output_DETAILS_unit_number, '(A,A12,F9.3,F8.1,F8.2,F8.2,F8.1,F9.2,F9.0,F8.2,F8.4,F9.2,F15.1,F12.3,F11.8)', advance = 'no') Date(0), Hour_min(0), Hour_dec(0), Curr_sim_time/60., &
        Ins(0), Tamb(0), rh_amb(0), wind(0), Patm(0), Tf_in(0), Xf_in(0), Tf(Nx), Usef_Q_rate, Efficiency, mdot_a(0)
    ...
    do while(Curr_sim_time < Total_sim_time)
        ...
        UsefulEnergyTotal = UsefulEnergyTotal + (Tf(Nx)-Tf_inlet)*mdot_f*c_f*dt    
        ...
        if (Next_store_time - Curr_sim_time < dt) then
            write(output_unit_number, '(A,A12,F9.3,F8.1,F8.1,F8.1,F8.1,F8.0,F8.4,F13.3,F12.8)') Date(index), Hour_min(index), Hour_dec(index), &
                Curr_sim_time/60., Ins(index), Tamb(index), wind(index), Patm(index), Xf_in(index), Tf(Nx), mdot_a(index)
            print *, "saving..."
        ...
        if (Next_store_time_details - Curr_sim_time < dt) then
            write(output_DETAILS_unit_number, '(A,A12,F9.3,F8.1,F8.2,F8.2,F8.1,F9.2,F9.0,F8.2,F8.4,F9.2,F15.1,F12.3,F11.8)', advance = 'no') Date(index), Hour_min(index), Hour_dec(index), Curr_sim_time/60., &
            Ins(index), Tamb(index), rh_amb(index), wind(index), Patm(index), Tf_in(index), Xf_in(index), Tf(Nx), Usef_Q_rate, Efficiency, mdot_a(index)
    ...
    write(output_DETAILS_unit_number, '(A,F15.2)') "Total Useful energy (J):", UsefulEnergyTotal 
    
    write(output_DETAILS_unit_number, '(A,F15.3)') "Collector efficiency:", Col_eff

    
    deallocate(Date,Hour_min,Hour_dec,time_row,Ins,Tamb,rh_amb,wind,Patm,Tf_in,Xf_in,Tdewp,mdot_a,x_position)
