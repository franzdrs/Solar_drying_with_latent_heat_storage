...
from numpy import log, zeros, linspace, ones, where, amax, mean
...
Slab_width = 0.5        # Height of PCM in slab when all PCM is solid (m)
...
Duct_width = Slab_width*expansion  #This would be equal to the slab width when the whole PCM is liquid, due to the expansion, and would
...
outfileT.write("\n")
outfileT.write( "N_slabs:%-8d Slab_Length:%-8.2f Slab_width:%-8.2f Slab_thickness:%-8.2f PCM_mass:%-8.2f Air_gap:%-8.2f T0:%-8.1f Ta0:%-8.1f" % (N_slabs, Slab_length, Slab_width, Slab_thickness, Mass_pcm, Air_gap, T0, Ta0))
outfileT.write("\n")
...
outfileT.write("%-12s%-9s%-9s%-8s%-8s%-8s%-8s%-8s%-8s%-9s%-11s%-11s"  % ("Date", "Hour_min", "Hour_dec", "t_min", "Ins", "Tamb", "Wind", "Patm", "X", "Tout_avg", "Lambda_avg", "mdot_a"))
outfileT.write("\n")
######################
...
outfileT_details.write( "N_slabs:%-8d Slab_Length:%-8.2f Slab_width:%-8.2f Slab_thickness:%-8.2f PCM_mass:%-8.2f Air_gap:%-8.2f T0:%-8.1f Ta0:%-8.1f" % (N_slabs, Slab_length, Slab_width, Slab_thickness, Mass_pcm, Air_gap, T0, Ta0))
outfileT_details.write("\n")
...
outfileH_details.write( "N_slabs:%-8d Slab_Length:%-8.2f Slab_width:%-8.2f Slab_thickness:%-8.2f PCM_mass:%-8.2f Air_gap:%-8.2f T0:%-8.1f Ta0:%-8.1f" % (N_slabs, Slab_length, Slab_width, Slab_thickness, Mass_pcm, Air_gap, T0, Ta0))
outfileH_details.write("\n")
...
while Curr_sim_time < Total_sim_t:
    ...
    pcm_width = Slab_width + Lambda_avg*(Duct_width - Slab_width)                   # This PCM width is the actual "height" of the pcm inside the slabs as it melts
    ...
    Tout_avg_stor_dt =  (Tout_avg_stor_dt*(Count_stor_dt - 1) + Ta_out_mixed)/Count_stor_dt    # This is used to calculate the average outlet T during the store_dt.
                                                                                         # It calculates the mean of all Tout values between store_dts to give a value
                                                                                         # of mean Tout for the whole storage period. For example if i store data evewry minute
    ...
    if (Next_store_time - Curr_sim_time) < dt:
        ...
        outfileT.write("%-9.3f" % Tout_avg_stor_dt)
        outfileT.write("%-11.3f" % Lambda_avg)
