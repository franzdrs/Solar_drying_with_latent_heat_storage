...
program LHS_2D
    ...
    double precision :: Slab_length, Slab_width_s, Slab_thickness, Slab_half_thickness, Air_gap, Half_air_gap, &
    ...
    double precision, allocatable :: H(:,:), H_new(:,:), Lambda(:,:), grid_weights(:,:)
    ...
    double precision :: Next_store_time, Next_store_time_details, Count_stor_dt, Ta_out_mixed, Total_enthalpy, Avg_temperature
    ...
    character(len=100) :: output_file = 'LHS_Output_SAH_20m2_Slab3m_Stutt_7_8Aug2022.txt'
    character(len=100) :: output_DETAILS_T_file = 'LHS_Output_SAH_20m2_Slab3m_Stutt_7_8Aug2022_DETAILS_T.txt'
    ...
    Slab_width_s = 0.5                    ! Width/height of PCM in the slab when all PCM is solid (m)
    ...
    Duct_width = Slab_width_s*expansion   ! This would be equal to the PCM width/height in the slab when the whole PCM is liquid
    ...
    allocate(grid_weights(0:N,0:M))     ! Defines size of matrix for grid weights
    ...
    write(output_unit_number, '(A,I8,2(A,F8.2),3(A,F8.3),2(A,F8.1))') "N_slabs:", N_slabs, " Slab_Length:", Slab_length, &
        " Slab_width_s:", Slab_width_s, " Slab_thickness:", Slab_thickness, " PCM_mass:", mass_PCM, " Air_gap:", Air_gap, " T0:", T0, " Ta0:", Ta0
    ...
    write(output_unit_number, '(A,F8.1,2(A,F8.5))') "dt:", dt, " dx:", dx, " dy:", &
        dy
    write(output_unit_number, '(A12,A12,F9.3,F8.1,F8.1,F8.1,F8.1,F8.0,F8.4,F13.3,F11.2,F12.1,F11.3,F11.8)') Date(0), Hour_min(0), Hour_dec(0), Curr_sim_time/60., &
        Ins(0), Tamb(0), wind(0), Patm(0), X(0), Ta(M), Avg_temperature, Total_enthalpy, Lambda_avg, mdot_a(0)

    write(output_unit_number, '(A,A12,A9,A8,A8,A8,A8,A8,A8,A13,A11,A12,A11,A11)') "Date", "Hour_min", "Hour_dec", "t_min", "Ins", "Tamb", "Wind", "Patm", "X", &
         "Ta_out", "Avg_Tpcm", "Tot_Enthlp", "Lambda_avg", "mdot_a"
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do while (Curr_sim_time < Total_sim_t)
        ...
        pcm_width = Slab_width_s + Lambda_avg*(Duct_width - Slab_width_s)   ! This PCM width is the actual width/height of the PCM inside
        ...
        if (Next_store_time - Curr_sim_time < dt) then
            write(output_unit_number, '(A12,A12,F9.3,F8.1,F8.1,F8.1,F8.1,F8.0,F8.4,F13.3,F11.2,F12.1,F11.3,F11.8)') Date(indx), Hour_min(indx), Hour_dec(indx), &
                Curr_sim_time/60., Ins(indx), Tamb(indx), wind(indx), Patm(indx), X(indx), Ta_out_mixed, Avg_temperature, &
                Total_enthalpy, Lambda_avg, mdot_a(indx)
            print *, "saving..."
